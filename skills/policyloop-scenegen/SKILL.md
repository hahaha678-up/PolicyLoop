---
name: policyloop-scenegen
description: >
  Generate USD scene files for PolicyLoop from natural language descriptions.
...
compatibility: >
  Requires a PolicyLoop project with assets/scenes/base_empty.usda and assets/objects/object_catalog.json.
metadata:
  author: nvidia
  version: "1.0.0"
---
...
I use the object catalog at `assets/objects/object_catalog.json` (24 objects) built from these directories:

| Directory | Count | Examples |
|-----------|-------|---------|
| `assets/objects/basic/` | 4 | colored blocks |
| `assets/objects/fruits_veggies/` | 2 | lemon, orange |
| `assets/objects/handal/` | 1 | spoon |
| `assets/objects/hot3d/` | 5 | cube, plates, bottle, mouse, remote |
| `assets/objects/ycb/` | 12 | banana, bowl, cups, bottles, cans, tools |
...
4. **Resolve the object catalog.**
   ...
   - **Tell the user which catalog you're using** and how many objects are available (e.g., "Using 24 objects from `assets/objects/object_catalog.json`" or "Using 24 default objects + 5 custom objects from `/path/to/custom/`").
...
The skill uses the same predicate-based solver from `policyloop/scene_gen/llm_scene_gen/` to compute collision-free placements. This is a 4-step pipeline that does **not** require IsaacSim — it runs with pure Python + numpy + scipy.

### Step 1: Generate predicates JSON

Based on the user's description, generate a JSON structure with object selections and predicates:
...
```python
import json, sys
from policyloop.scene_gen.llm_scene_gen import (
...
**1. Create a task** — follow [Task Authoring](../../docs/task.md) to define a task file that references this scene and uses the retained conditionals.

**2. Verify in simulation** — run the environment smoke test (no policy server needed) once you have a task. It checks environment creation, reset, and five simulation steps:
```bash
python -m pytest tests/test_run_empty.py -v --task <TaskClassName>
```
