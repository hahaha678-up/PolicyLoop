...
### Run a simulation smoke test

Create and reset one environment, then execute five empty-action steps. This checks simulation startup and the termination interface; it does not wait for task success or a full episode timeout:

```bash
uv run pytest tests/test_run_empty.py -v                   # default: BananaInBowlTask
uv run pytest tests/test_run_empty.py -v --task RubiksCubeAndBananaTask
```
...
This does not change the installed driver. Leave the variable unset on machines without this version-reporting issue. Test outputs use temporary directories, and the test command returns a nonzero exit code on failure.
