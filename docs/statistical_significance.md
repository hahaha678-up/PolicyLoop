...
where `k` is the number of successful episodes and `n` the total. The 95% interval is `[Beta.ppf(0.025), Beta.ppf(0.975)]` of that posterior. This is the same interval shown in the `95% CI` column of the default summary and in the `LCB %` / `UCB %` columns in CSV mode (see [Analysis and Results Parsing](analysis.md#output-fields)).
