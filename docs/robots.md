...
The ground heights of the five bundled scenes are checked by `tests/test_scene_ground.py`. Three scenes use −0.697 m; `bananas_5_grey_bin.usda` and `rubiks_cube_banana_bowl.usda` use −0.65 m. The table rests on the ground, so changing its height changes the workspace geometry and can invalidate comparisons with existing recordings.

Custom floor-standing robot configurations can declare `root_z_above_ground` to align the robot base with the scene ground. The built-in DROID and Franka configurations use the table fixture.
