...
task_envs = get_envs(task=["BananaInBowlTask", "RubiksCubeAndBananaTask"])  # Multiple tasks
tag_envs = get_envs(tag="semantics")                # All environments with a tag
...
**Choosing `num_envs`:** Choose a parallel count that fits both simulation and policy inference in available VRAM. See the [parallel environment guide](env_vram_size_guide.md). Increase `--num-runs` if a smaller batch is needed to reach the desired episode count:
...
| `--policy BACKEND` | Pi0 variant (`pi0`, `pi0_fast`, `pi05`, `paligemma`, `paligemma_fast`); GR00T uses its own runner | `pi05` |
...
| `--headless` | Run without live display window. **Recommended for multi-task runs** — see [GPU VRAM leak in non-headless mode](debug.md#known-issues) | `False` |
...
# Run specific tasks
python policies/pi0_family/run.py --task BananaInBowlTask RubiksCubeAndBananaTask

# Run tasks by tag
python policies/pi0_family/run.py --tag semantics
...
# Render with the path tracer (see Renderer Selection)
...
**Runtime:** The default runner evaluates the eight bundled tasks. Episode time limits come from each task definition (30–90 simulation seconds). Wall time depends on inference latency, rendering, parallelism and early termination; inspect the current run output rather than using a fixed full-benchmark estimate.
