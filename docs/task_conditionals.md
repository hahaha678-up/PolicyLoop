...
- [Run condition checks](#run-condition-checks)
...
Run the retained conditional regression tests in the simulator environment:

```bash
python -m pytest tests/test_grouped_conditionals.py tests/test_gripper_conditionals.py tests/test_runtime_boundary.py -v
```

The runtime tests exercise the condition and event interfaces for all eight bundled tasks.
