...
This guide covers how to register PolicyLoop tasks as runnable Gymnasium environments with your specific robot, observations, actions, and simulation settings. You do **not** need to modify PolicyLoop — registration can be done from your own repository.
...
> **If you're using DROID with joint-position actions**, PolicyLoop ships a ready-to-use registration. You can skip to [Evaluating a New Policy](policy.md), which shows how to use the built-in registration directly.
...
**For DROID users:** PolicyLoop ships a proprioception observation config and a helper that generates an image observation config from any list of camera configs. If these work for your policy, you can skip this step and import/build them directly in Step 2:
...
from policyloop.robots.droid import ProprioceptionObservationCfg
```

Presets live in `policyloop/registrations/droid/camera_presets.py`: `WRIST`, `WRIST_LEFT`, `WRIST_RIGHT`, `WRIST_LEFT_RIGHT`, `WRIST_LEFT_RIGHT_HEAD`, `LEFT_RIGHT`. Pick one or pass your own list. Viewport-only cameras (used for recorded video, not policy input) are attached separately inside the registration function and do not belong in these presets.
...
The camera names (`over_shoulder_left_camera`, `wrist_cam`) are mapped from the camera configurations you provide in Step 2. You can mix and match — use the built-in `ProprioceptionObservationCfg` from `policyloop.robots.droid` with your own custom `ImageObsCfg`, or vice versa.
...
The examples above register PolicyLoop's built-in benchmark tasks (from `TASK_DIR`). If you've created your own tasks (see [Tasks](task.md)), you can register them in the same function.
...
For complete working examples inside the PolicyLoop repo:

- [`policyloop/registrations/example/auto_env_registration.py`](../policyloop/registrations/example/auto_env_registration.py) — Basic registration example
- [`policyloop/registrations/droid/auto_env_registrations_jointpos.py`](../policyloop/registrations/droid/auto_env_registrations_jointpos.py) — Full DROID robot registration
...
```python
from policyloop.core.environments.factory import get_envs_by_task, get_envs_by_tag

envs = get_envs_by_task("BananaInBowlTask")
envs = get_envs_by_tag("semantics")
```
...
Scene parameters (each can live in your own repository — no need to modify PolicyLoop):
