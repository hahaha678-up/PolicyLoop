...
Usage:
    ...
    python generate_task_metadata.py --subfolders benchmark

    # Import as a module
    from policyloop.tasks._utils.generate_task_metadata import generate_task_metadata
    generate_task_metadata("/path/to/tasks", "/path/to/output", subfolders=["benchmark"])
...
Command Line Arguments:
    ...
    --subfolders: List of subfolder names to include (e.g., --subfolders benchmark)
...
def generate_task_metadata(tasks_folder: str, output_folder: str = None, include_images: bool = False, subfolders: List[str] = None):
    ...
    from policyloop.tasks._utils.load_task_info import scan_tasks_folder
    ...
    if include_images and os.path.isdir(image_dir):
        from policyloop.core.utils.csv_utils import add_images_to_csv
    ...
    try:
        ...
        if subfolders is not None:
            ...
            description = f"This table contains metadata for tasks in `policyloop/tasks`.\n\n**Filtered by subfolders:** {subfolder_list}\n\n**Total Tasks: {total_tasks}**"
        else:
            description = f"This table contains metadata for all tasks in `policyloop/tasks`.\n\n**Total Tasks: {total_tasks}**"
...
if __name__ == "__main__":
    ...
    from policyloop.core.utils.csv_utils import save_markdown_table
    import argparse
    from policyloop.constants import TASK_DIR, DEFAULT_TASK_SUBFOLDERS
    ...
    parser.add_argument("--subfolders", nargs="+", default=None,
                       help="List of subfolder names to include (e.g., --subfolders benchmark). If not specified, all subfolders are included. When using the default --tasks-folder, defaults to DEFAULT_TASK_SUBFOLDERS.")
