...
"""Policy episode runner for PolicyLoop.
...
from policyloop.constants import VISUALIZE, get_output_dir
from policyloop.core.logging.results import get_all_env_subtask_infos
from policyloop.core.observations.observation_utils import unpack_image_obs, unpack_viewport_cams
...
def run_episode(
    ...
    *,
    ...
    Args:
        env: The environment instance (PolicyLoopEnv with num_envs >= 1)
        ...
        enable_gt_state: Attach per-env ground-truth sim state to the obs dict
            ...
            ``policyloop/eval/gt_state.py`` for the schema).
    ...
    try:
        for step in tqdm(range(max_steps)):
            ...
            # sim state can pick out their env's entry (see policyloop/eval/gt_state.py).
            ...
            # PolicyLoopEnv freezes terminated envs and exports recordings automatically
