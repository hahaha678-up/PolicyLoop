...
def run_evaluation(
    ...
    *,
    ...
    from policyloop.constants import PACKAGE_DIR, get_timestamp, set_output_dir
    ...
    from policyloop.core.utils.adaptive_sampling import count_task_episodes, should_continue_sampling
    from policyloop.core.utils.print_utils import print_experiment_summary
    from policyloop.eval.episode import run_episode
    from policyloop.eval.summarize import summarize_run
    ...
    for task_env in task_envs:
        ...
        if policyloop.constants.ENABLE_SUBTASK_PROGRESS_CHECKING and getattr(env_cfg, "subtasks", None) is None:
            print(
                f"\033[93m[PolicyLoop] WARNING: Subtask tracking is enabled but task `{task_env}` "
        ...
        while True:
            if is_adaptive:
                ...
                if not should_continue_sampling(
                    ...
                    print(
                        f"\033[96m[PolicyLoop] Task `{task_env}` adaptive stop at {n_so_far} "
            ...
            print(
                f"\033[96m[PolicyLoop] Running {run_name}: '{env_cfg.instruction}' "
