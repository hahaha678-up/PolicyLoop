...
:func:`policyloop.eval.episode.run_episode` calls :meth:`GroundTruthStateExporter.export_all`
...
detection) are computed by consumers from this per-step stream.
...
class GroundTruthStateExporter:
    ...
    Args:
        env: The ManagerBasedRLEnv (PolicyLoopEnv) instance.
    ...
    def _export_object_completed(self, sm: Any, env_id: int) -> dict[str, bool]:
        ...
        The current subtask uses policyloop's tracked CSM state (authoritative);
        ...
        Lets a consumer whose subgoal order differs from policyloop's internal
