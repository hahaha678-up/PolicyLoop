...
This is the canonical list of robot embodiments that ship with PolicyLoop. For how to *use* a robot
...
arm, plus a 720p wrist camera whose intrinsics are calibrated to match pi05 training data.
...
- **Registrations:** `policyloop/registrations/droid/` (jointpos, abs-IK, rel-IK, lighting/background variations)
...
```python
from policyloop.robots.franka import FrankaCfg                      # standard PD
from policyloop.robots.franka_high_pd import FrankaCfg              # high PD, gravity disabled
from policyloop.robots.franka_definitions import FrankaJointPositionActionCfg, contact_gripper
```

---

Also in this folder: `delta_actions.py`, a helper that converts a target EE pose into a relative
(delta) pose action. It does not define an action space.
