...
from policyloop.constants import BENCHMARK_TASK_CATEGORIES, TASK_DIR
from policyloop.core.task.status import StatusCode, get_status_name
from policyloop.core.utils.file_utils import load_file  # noqa
...
def summarize_experiments_by_category_with_attributes(episode_results: list[dict] | str,
    ...
    Args:
        ...
        remap: Dictionary mapping attribute names to category names.
               If None, uses BENCHMARK_TASK_CATEGORIES from policyloop.constants
...
def _load_task_metadata() -> list[dict]:
    """Read policyloop/tasks/_metadata/task_metadata.json or raise if missing."""
    metadata_path = os.path.join(TASK_DIR, "_metadata", "task_metadata.json")
    if not os.path.exists(metadata_path):
        raise FileNotFoundError(
            f"Task metadata file not found: {metadata_path}. "
...
def summarize_experiments_by_instruction_type(episode_results: list[dict] | str, VERBOSE=False, csv=False, csv_compact=False, show_metrics=True):
    ...
    print("[PolicyLoop] summarize_experiments_by_instruction_type is not yet implemented.")
...
def summarize_experiments_by_num_objects(episode_results: list[dict] | str,
    ...
    as reported by policyloop/tasks/_metadata/task_metadata.json. The list is taken
...
def summarize_experiments_by_task_length(episode_results: list[dict] | str,
    ...
    compute_difficulty_score in policyloop/core/task/subtask_utils.py.
    """
    _summarize_experiments_by_task_field(
        episode_results,
