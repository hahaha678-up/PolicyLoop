...
from policyloop.core.logging.streaming_hdf5_handler import StreamingHDF5DatasetFileHandler
...
class PolicyLoopRecorderManager(RecorderManager):
    ...
    def set_flush_interval(self, interval: int, verbose: bool = False):
        ...
        if interval > 0:
            print(f"[PolicyLoopRecorderManager] Auto-flush enabled: every {interval} steps")
        else:
            print(f"[PolicyLoopRecorderManager] Auto-flush disabled")
    ...
    def _active_env_ids(self) -> list[int]:
        ...
        upstream RecorderManager behavior for non-PolicyLoopEnv subclasses.
        """
