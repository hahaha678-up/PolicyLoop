...
try:
    ...
    def _get_scales_usd_float3_safe(self, indices=None) -> torch.Tensor:
        ...
        which is how all PolicyLoop scene assets author it. Only ``double3`` scales
...
import policyloop.core.utils.usd_utils as usd_utils
from policyloop.core.sensors.contact_sensor_utils import (
    get_batch_contact_sensor,
    get_contact_sensor,
    get_contact_sensor_with_order,
)
from policyloop.core.utils import vis_utils
from policyloop.core.utils.debug_utils import get_caller_info
...
class WorldState:
    ...
    # PolicyLoopEnv._reset_idx invokes reset_predicate_state(env_ids) at every
    ...
    def reset_predicate_state(self, env_ids) -> None:
        ...
        Called by PolicyLoopEnv._reset_idx at every path that actually resets envs.
    ...
    def get_frame_pose(self, frame: str, frame_cfg_name: str="frames", as_matrix: bool=False, env_id: int | None = None) -> torch.Tensor | tuple[torch.Tensor, torch.Tensor]:
        ...
        if as_matrix:
            from policyloop.core.utils.geometry_utils import pose_from_pos_quat
    ...
    def get_objects_in_contact_with(
        ...
        if batch_sensor is None:
            ...
            if policyloop.constants.VERBOSE:
                from policyloop.core.sensors.contact_sensor_utils import get_contact_sensors
