...
from policyloop.constants import DEBUG
from policyloop.core.task.hull_check import build_local_hull, point_in_hull
from policyloop.core.utils.geometry_utils import spatial_condition_check_vector_based
from policyloop.core.utils.transform_utils import transform_pose_from_w_to_b_vectorized
...
def get_task_conditional_func(func_name: str):
    ...
    from policyloop.core.utils.function_loader import load_callable_from_module
...
def in_opentop_container(world, inside_obj: str, outside_obj: str, tolerance: float = 0.0, env_id: int | None = None):
    ...
    This public alias supports task configurations using open-top containment.
    """
    result = _obj_centroid_in_container(world, inside_obj, outside_obj, env_id)
    if DEBUG and env_id is not None:
