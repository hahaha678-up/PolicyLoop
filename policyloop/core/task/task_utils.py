...
def load_task_from_file(task_file_path: str, allow_multiple: bool = False) -> Task | list[Task]:
    ...
    Args:
        task_file_path: Path to the task file (e.g., 'banana_in_bowl_task.py')
...
def resolve_task_path(task: str, task_dir: str | Path) -> tuple[str, str]:
    ...
    Examples:
        resolve_task_path("/path/to/banana_in_bowl_task.py", task_dir)
        # Returns ("/path/to/banana_in_bowl_task.py", "BananaInBowlTask")

        resolve_task_path("banana_in_bowl_task.py", task_dir)
        # Returns ("/full/path/to/banana_in_bowl_task.py", "BananaInBowlTask")

        resolve_task_path("BananaInBowlTask", task_dir)
        # Returns ("/full/path/to/banana_in_bowl_task.py", "BananaInBowlTask")
    """
    # Check cache first
    ...
    # Case 3: Task class name - searches in task_dir for all files, and loop through each file to find the class name.
        ...
        # Restrict name lookup to the default task folders so an unrelated
        # custom definition cannot shadow a bundled task with the same name.
        from policyloop.constants import DEFAULT_TASK_SUBFOLDERS  # noqa: PLC0415

        # Most task classes follow the repository's ClassName -> class_name.py
        # convention. Resolve that exact file first so selecting one task does
        # not import every task module (and consequently open every scene USD).
        conventional_name = re.sub(r"(?<!^)(?=[A-Z])", "_", task).lower() + ".py"
        for subfolder in DEFAULT_TASK_SUBFOLDERS:
            candidate_file = task_dir / subfolder / conventional_name
            if not candidate_file.is_file():
                continue
            try:
                task_class = load_task_from_file(str(candidate_file), allow_multiple=False)
                if not isinstance(task_class, list) and task_class.__name__ == task_class_name_to_find:
                    result = (str(candidate_file), task_class_name_to_find)
                    _resolve_task_cache[cache_key] = result
                    return result
            except Exception:
                # Preserve the existing exhaustive fallback for unconventional
                # or invalid task modules.
                pass
