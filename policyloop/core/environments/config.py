...
from policyloop.core.environments.base import PolicyLoopDefaultEnvCfg
from policyloop.core.environments.scene_fixture import (
    FRANKA_TABLE_FIXTURE,
    robot_cfg_above_ground,
    scene_without_table_fixture,
    table_fixture_asset,
)
from policyloop.core.sensors.contact_sensor_utils import create_contact_sensors
from policyloop.core.task.task import Task
from policyloop.core.task.task_utils import load_task_from_file
...
def generate_scene_env_cfg(task_class: Task,
    ...
    Args:
        ...
        robot_cfg: Robot configuration class to include. Two optional class-attribute
            ...
            height plus that offset. See policyloop.core.environments.scene_fixture.
...
def generate_task_env_cfg(task_class: Task,
                         ...
                         object_state_obs: bool = False) -> Type[PolicyLoopDefaultEnvCfg]:
    """
    ...
    """
    from policyloop.core.task.subtask_utils import compute_difficulty_score, count_subtasks

    if object_state_obs:
        from isaaclab.managers import ObservationGroupCfg as ObsGroup

        from policyloop.core.observations.observation_utils import generate_object_state_obs, generate_obs_cfg
    ...
    @configclass
    class GeneratedTaskEnvCfg(PolicyLoopDefaultEnvCfg):
...
def auto_generate_task_env(task_file_path: str,
                          ...
                          actions_cfg = None,
                          **env_kwargs) -> Type[PolicyLoopDefaultEnvCfg]:
    """
...
def register_generated_env(task_env_cfg: PolicyLoopDefaultEnvCfg, env_name: str = None):
    """
    ...
    gym.register(
        id=env_name,
        entry_point="policyloop.core.environments.env:PolicyLoopEnv",
        kwargs={
...
def generate_env_cfg_from_task(task_file_path: str,
                    ...
                    env_postfix: str="",
                    register: bool = True,
                    **kwargs) -> tuple[Type[PolicyLoopDefaultEnvCfg], str]:
    """
...
def print_env_cfg(env_cfg: PolicyLoopDefaultEnvCfg, prefix=""):
