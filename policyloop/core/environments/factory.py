...
from policyloop.core.environments.config import generate_env_cfg_from_task, print_env_cfg
from policyloop.core.task.task_utils import resolve_task_path
...
class EnvFactory:
    ...
    def create_env_cfg(self,
        ...
        Args:
            task: Task identifier, can be:
                - Full path: "/path/to/banana_in_bowl_task.py"
                - Filename: "banana_in_bowl_task.py"
        ...
        Examples:
            ...
            factory.create_env_cfg("BananaInBowlTask")

            # Using full file path
            factory.create_env_cfg("/path/to/tasks/banana_in_bowl_task.py")

            # With tags and postfix
            factory.create_env_cfg("BananaInBowlTask", tags="my_tag", env_postfix="v2")
    ...
    def batch_create_env_cfgs(self,
        ...
        Examples:
            ...
            factory.batch_create_env_cfgs(
                ["BananaInBowlTask", "RubiksCubeAndBananaTask", "RubiksCubeOrBananaTask"],
                tags="fruit_tasks",
            )

            # With task_subdirs to limit search scope
            factory.batch_create_env_cfgs(
                ["BananaInBowlTask", "RubiksCubeAndBananaTask"],
                tags="fruit_tasks",
            ...
            factory.batch_create_env_cfgs(
                ["BananaInBowlTask", "/path/to/CustomTask.py"],
                tags="my_tag",
                env_postfix="v2",
            )
        ...
        if task_subdirs is not None:
            ...
            for task_file in task_files:
                ...
                try:
                    from policyloop.core.task.task_utils import get_task_class_name_from_file
    ...
    def auto_discover_and_create_cfgs(self,
        ...
        if tasks is not None:
            ...
            print(f"\033[96m[PolicyLoop] Registering {len(task_list)} task(s): {task_list}\033[0m")
...
def create_env_cfg(task: str, task_dir=None, **kwargs) -> type:
    ...
    Examples:
        # Using task name (looks in task_dir)
        create_env_cfg("BananaInBowlTask")

        # Using full file path
        create_env_cfg("/path/to/tasks/banana_in_bowl_task.py")

        # With tags and postfix
        create_env_cfg("BananaInBowlTask", tags="my_tag", env_postfix="v2")
...
def get_envs(task: str | list[str] | None = None,
    ...
    Examples:
        ...
        envs = get_envs(task=["BananaInBowlTask", "RubiksCubeAndBananaTask"])

        # Get a specific environment
        envs = get_envs(env="BananaInBowlTask")
